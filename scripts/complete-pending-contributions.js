...
async function completePendingContributions() {
  try {
    ...
    const { data: contributions, error: fetchError } = await supabase.rpc(
    ...
    console.log(
      `Found ${contributions?.length || 0} pending contributions with checkout sessions`,
    );
    ...
    if (contributions && contributions.length > 0) {
      ...
      contributions.forEach((c) => {
        console.log(
          `  - ${c.backer_email}: $${c.amount} (${c.stripe_checkout_session_id})`,
        );
      });
      console.log("");
    }
    ...
    for (const contribution of contributions || []) {
      ...
      const { error: updateError } = await supabase.rpc(
        "complete_contribution",
        {
          ...
          p_payment_intent_id:
            contribution.stripe_payment_intent_id || "manual_completion",
      ...
      if (updateError) {
        console.error(
          `Error completing contribution ${contribution.id}:`,
          updateError,
        );
      } else {
